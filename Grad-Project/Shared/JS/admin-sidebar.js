document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      const response = await fetch("../../Shared/HTML/admin-sidebar.html");
              ...
              <div style="
                ...
                <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px;text-align:left;">
                  ...
                  <div style="border-left:2.5px solid rgba(139,92,246,0.6);border-radius:0 12px 12px 0;background:rgba(139,92,246,0.05);padding:13px 16px;">
                    ...
                    <p style="color:rgba(255,255,255,0.75);font-size:13px;line-height:1.75;margin:0;">The <strong style="color:#a78bfa;">"i"</strong> at the center represents <em style="color:#fff;">Information</em> and the <em style="color:#fff;">IAU'ers identity</em>, reflecting the knowledge that connects everyone on the platform.</p>
                  </div>
                ...
                <p style="text-align:center;font-size:11px;color:rgba(255,255,255,0.22);margin:0;">IAU · College of Business Administration · Graduation Project 2026</p>
              </div>
            </div>
          `);
          document.getElementById("closeConceptModal").addEventListener("click", () => document.getElementById("iauersConceptModal").remove());
          document.getElementById("iauersConceptModal").addEventListener("click", (e) => { if (e.target === e.currentTarget) e.currentTarget.remove(); });
        });
      }
